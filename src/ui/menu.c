...
#define MENU_X1          290
#define MENU_PAD         10
#define MENU_TITLE_Y     58
#define MENU_SEP_Y       70
#define MENU_ITEMS_Y     80
#define MENU_ROW_HEIGHT  20
#define MENU_LABEL_X     44
#define MENU_VALUE_X     170
#define MENU_FOOTER_PAD  8
...
void menu_open(Menu *menu) {
    ...
    for (int i = 0; i < menu->item_count; i++) {
        menu->snapshot[i] = menu->items[i].selected;
    }
    menu->cursor = 0;
...
void menu_close(Menu *menu, bool apply) {
    if (!apply) {
        // Revert to snapshot
        for (int i = 0; i < menu->item_count; i++) {
            menu->items[i].selected = menu->snapshot[i];
        }
    }
    menu->is_open = false;
}
...
void menu_update(Menu *menu) {
    if (!menu->is_open || menu->item_count == 0) return;

    joypad_buttons_t pressed = joypad_get_buttons_pressed(JOYPAD_PORT_1);

    // D-pad up/down: move cursor
    if (pressed.d_up) {
        menu->cursor = (menu->cursor - 1 + menu->item_count) % menu->item_count;
    }
    if (pressed.d_down) {
        menu->cursor = (menu->cursor + 1) % menu->item_count;
    }

    // D-pad left/right: cycle option
    MenuItem *cur = &menu->items[menu->cursor];
    if (pressed.d_left) {
...
void menu_draw(const Menu *menu) {
    ...
    // Compute menu height
    int footer_y = MENU_ITEMS_Y + menu->item_count * MENU_ROW_HEIGHT + MENU_FOOTER_PAD;
    ...
    for (int i = 0; i < menu->item_count; i++) {
        ...
        text_draw(&label_cfg, menu->items[i].label);
        ...
        const char *opt = menu->items[i].options[menu->items[i].selected];
...
int menu_get_value(const Menu *menu, int item_index) {
    if (item_index < 0 || item_index >= menu->item_count) return 0;
    return menu->items[item_index].selected;
}
