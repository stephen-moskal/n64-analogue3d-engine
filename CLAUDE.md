...
- **Audio mixing runs right after `display_get()`** (`audio_poll()` in `engine.c`). Each mixed buffer is a high-priority RSP job; since Phase 3 S2 the CPU queues it and carries on (`SND_MIX_ASYNC`, `mixer_try_play`), because waiting for it cost 6–9.5 ms per frame near the budget (D38). `snd_starved` counts audio gaps. Rerun Bench = Audio when the frame loop or the RSP load changes.
- **Frame queue (Phase 3 S2, docs/ENGINE.md):** the engine records each frame's drawing into one of two rspq queues and submits it at the end of the frame, so the CPU never stalls on libdragon's SYNC_FULL hold (48 pillars 40.6 → 60 FPS). While it records (scene draw and overlay), drawing code must not call `rspq_wait()` or libdragon's deferred calls (they assert); free surfaces with `engine_call_after_rdp()`. Each queue keeps its largest frame's size (~100 B per triangle) until a scene switch releases it (`engine_frame_queue_release()`). Where the CPU now keeps up with 60 FPS, frames queue ahead in the triple buffer (+~1 vblank of lag; Latency = Lowest avoids it). Sound effects must be mono (one mixer channel per voice) and share one encoding.
...
src/scenes/benchmark_scene.c   stress test (All = 28 steps, plus Layout, Overload, Audio, UI, Latency, Mesh, RSP); BENCH / BENCH_PRESENT /
