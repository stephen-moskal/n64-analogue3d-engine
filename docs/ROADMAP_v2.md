...
| 3 | Graphics features independent of Tiny3D | **in progress** — §7 (stages S0–S14; S0 2026-09-25) | Vertex cache, Gouraud, sprite animation, CI4/TMEM residency, fonts, VI options, decals, skybox, sorted transparency — each with a BENCHMARKS row |
...
## 7. Phase 3 — Graphics features independent of Tiny3D — status: in progress (S0 2026-09-25)
...
|---|---|---|---|---|
| Post-transform vertex cache (each mesh vertex transformed once, not per triangle corner) | High | No | P3.1 (S3) | vertices are shared only within a face group, and the triangle submission costs more than the transforms: −12 to −21 % `mesh_tris` expected on pillars, −25 to −35 % on spheres (§7.2; the earlier −40–50 % estimate predates the measurement); prerequisite for Gouraud |
...
| S1 · Measurement | `BENCH_MESH` kind (normal / submission skipped / coverage with fog and textures), `make HEAP_PAD=N` (D37 probe), golden RDP captures (§7.4) | — | planned | transform vs submit split and the D37 verdict recorded; S3's target set |
...
| S3 · Vertex cache (P3.1, D26) | per-group vertex ranges; each visible group's vertices transformed once into a pinned scratch that libdragon's triangle formats read directly (§7.4) | stress meshes once S4 exists | planned | golden RDP captures byte-identical; `mesh_tris` per drawn triangle −12 % on pillars, −25 % on spheres; no step worse than +5 %; `LAYOUT_PAD=448` within ±2 % |
...
| S12 · Hygiene | the menu's text cache freed while closed and geometry blocks pooled (§6.3 heap items); D40; a Start-menu driver shared by the demo and the viewer; D37 follow-up; docs audit | — | planned | heap ≤ the start + declared feature costs |
...
**S1 · Measurement.**
- **`BENCH_MESH`** (not in All): param = variant × 1000 + pillars (16, 32, 64), interleaved like Layout.
  - Variant 0 draws normally.
  - Variant 1 skips triangle submission; the corners go to a sink the compiler cannot drop.
  - Variant 2 covers all four triangle formats and curved groups (fog on, textured boxes, spheres).
  - Compare `mesh_tris_us`; CPU averages include the D38 wait.
- **`make HEAP_PAD=N`** allocates N bytes before libdragon's buffers, moving the command buffers and framebuffers in RDRAM. If per-primitive costs move ≥ 3 % while `hot_text.py` and `hot_data.py` report the same layout, D37 is RDRAM placement.
- **`BENCH_RDPLOG=<step>`** captures one frame of a step. `rdp_log_to_hex.py` turns it into the golden dump S3 must reproduce byte for byte.
...
**Test plan:** the stage gate of §6.4 for every stage. Same-ROM A/B variants decide S1–S3, and S3 is checked byte for byte against S1's golden RDP captures. From S4, the host tests cover the mesh format and loader, and CI runs the tools' self-tests.
