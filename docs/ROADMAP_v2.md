...
| D40 | A menu tab whose items are all disabled would leave the cursor at -1: `find_next_enabled()` returns its start (-1) when no item is enabled, and `menu_change_value()` / `menu_switch_tab()` then read `items[-1]`. No tab reaches it today | `src/ui/menu.c` | found S12 (doc audit); fix with a host test at the next menu change |
