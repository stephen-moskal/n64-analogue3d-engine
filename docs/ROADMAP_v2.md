...
| 3 | Graphics features independent of Tiny3D | **in progress** — §7 (stages S0–S14; S0–S2a 2026-09-25) | Vertex cache, Gouraud, sprite animation, CI4/TMEM residency, fonts, VI options, decals, skybox, sorted transparency — each with a BENCHMARKS row |
...
## 7. Phase 3 — Graphics features independent of Tiny3D — status: in progress (S0–S2a 2026-09-25)
...
- **S2b.** Win back the latency: submit the frame in segments during the recording (a pool of queues, each run once; flushes between the scene's draw phases and every few objects), so the RDP starts as soon as it is free. Release the queues' memory on scene switches (they keep their largest frame). Acceptance: Bench = Latency and objects 48/64 lag at or below S1's, S2a's throughput kept.
