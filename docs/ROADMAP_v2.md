...
**In progress:** Phase 2, engine hardening (§6): S0–S4 verified on the A3D 2026-09-23; next S4b, the libdragon upgrade (D28).
...
| 2 | Engine hardening | **in progress** — §6 (stages S0–S13 + S4b); S0–S4 verified 2026-09-23 | Defect register closed (except items explicitly deferred), 0 B heap growth over 10 resets, heap −90 KB, demo CPU −15 %, menu frames within budget, one input path, `demo_scene.c` no longer owns menu semantics, physics verified-hw (§6.3) |
...
## 6. Phase 2 — Engine hardening — status: in progress (S0–S4 verified 2026-09-23)
...
| S4b | P2.13 **libdragon upgrade** (pulled forward from P4.0; D28) | in progress: S4b.1 libdragon upgrade ✔ verified-hw 2026-09-23; S4b.2 audio rework next | Move the submodule to current `preview` (≥ `7c57c409d`), `libdragon install`, `libdragon make clean` (asset formats), fix API drift in engine code (mixer, rdpq, sprites, fonts, `rspq_profile`), align `.libdragon/config.json` and `.devcontainer`; retry RSPQ_PROFILE (P1.7). Tiny3D stays in P4.0. | `libdragon/`, `src/audio/`, `src/ui/`, `src/render/`, `src/debug/`, SETUP.md, HARDWARE.md | CI green; full benchmark vs S4: no regression > 5 % unexplained; A3D checklist; no RSP crash across Reset Soak ×3, Menu Sweep and 30 min of use with the menu |
