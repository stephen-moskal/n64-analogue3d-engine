...
| D32 | In a boot-to-benchmark run (`BENCH=1`) on the A3D, step 0 ran ~40 % slower on both CPU and RDP, with the mixer at 4.1 ms instead of 0.09. That step is the first ~5 s after reset; every later step was normal. Cause unknown | boot sequence, `src/main.c` | open → S6 (frame loop); until then repeat or discard step 0 of a boot run. S6.2: `BOOT` rows (first 12 s, per second) show no slow start in a demo boot; the `BENCH=1` boot is checked next |
