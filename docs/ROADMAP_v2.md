...
| D8 | Every `Mesh` mallocs full capacity (16,384 + 2,048 B) regardless of size | `src/render/mesh_build.c` (`mesh_add_vertex`, `mesh_add_triangle`) | fixed S4 (`mesh_finalize()`: exact-size block; demo heap −101 KB) |
| D9 | Screen size (320/240) and guard-band constants duplicated across the renderer | `main.c:21-23`, `mesh.c:127-130,337`, `floor.c:21-24,121`, `shadow.c:12-15,90,166`, `particle.c:54-57,431`, `atmosphere.c:309-346`, `camera.c:164`, `scene.c:244`, `benchmark_scene.c:419` | fixed S6.1: `src/engine/engine_config.h` (verified-hw 2026-09-24) |
| D10 | Menu option strings live in `main.c`, their meanings in `demo_scene.c`, coupled by raw index with no bounds checks | `src/main.c:34-158`, `src/scenes/demo_scene.c:250-365` | **fixed S8 (verified-hw 2026-09-24):** `src/ui/settings.c/h`: each option's label, choices, default and values in one table (static asserts on every choice/value pair and tab size, `SettingId` instead of raw indices, typed accessors); `main.c` calls `settings_init()`; the demo lost its index macros, lookup tables and caches; host `test_settings.c` |
| D11 | `PhysicsWorld` is a demo static; `SceneObject.collider_handle` never used (demo keeps `obj_colliders[]`); scene texture manager unused | `src/scenes/demo_scene.c`, `src/scene/scene.c` | textures fixed S1; **fixed S7 (verified-hw 2026-09-24):** `Scene.physics`, objects own a collider and a body (`scene_object_set_collider/_body()`, removed with the object), `scene_update()` steps physics and syncs bodies → objects → colliders (`src/scene/scene_objects.c`, host-tested) |
| D12 | Three input paths: action layer, raw joypad in `menu.c`, raw START/shortcut/abort reads | `src/ui/menu.c` (`menu_update`), `demo_scene.c` (Start handling in `demo_update`), `debug_menu.c` (D-Up/D-Down shortcuts), `benchmark_scene.c` (abort) | **fixed S9 (verified-hw 2026-09-25):** one path. The engine polls (`input_poll`), every reader asks actions: menus and the dialog through the engine's modal UI context (`menu_update` / `textbox_update` take a `UiInput`), the shortcuts through the debug context below every game context, the benchmark's abort through its own context |
| D13 | `ParticleBlendMode` declared and set but the renderer hardcodes additive (point spawn already works implicitly) | `src/render/particle.c:341-346` | **fixed S10 (verified-hw 2026-09-25):** `particle_draw()` draws the alpha-blended emitters (`RDPQ_BLENDER_MULTIPLY`), then the additive ones, one blender change per frame; the demo's smoke is alpha-blended. Point spawn (`PARTICLE_SPAWN_POINT`) works and is documented in PARTICLES.md |
...
| D16 | Physics (Feature 9) only verified in ares | — | **fixed S11 (verified-hw 2026-09-25):** host tests at dt = 1/60, 1/30 and the NTSC frame times (at rest in 2.23 s, no tunnelling, steps follow the display's time); on the A3D the ball rests in 2.22–2.23 s at 60 and at 30 FPS (`BALL` rows). Found D39 |
| D17 | Committed generated `filesystem/` assets (a version-2 `.wav64` asserted `invalid version` at boot) | `.gitignore` | **fixed** d0d5bc0 |
| D18 | **Frame-overrun flicker on the Analogue 3D.** Frames over 16.7 ms (debug build + RDP validator + menu open) flicker in the lower screen area on the A3D (HDMI, 60 Hz 4K TV); ares shows only the FPS drop; the release ROM does not flicker. P1.3 measured the trigger: with the validator on, `menu_draw` peaks at 6–9 ms (text costs ~4× more under the validator). Mitigated by the validator being off at boot. **S0 Overload result: frame overruns alone do not flicker** (validator on or off, down to 26 FPS), so the flicker needs the menu on screen during an overrun: the menu drawing path (translucent background triangles, text) is the suspect | `src/main.c` frame loop, `src/ui/menu.c`, `src/ui/text.c` | S5.1: the cached menu removes the per-frame menu text; with the validator on the flicker no longer shows (A3D, 2026-09-24), but change frames and the validator's own cost still overrun 16.7 ms → S6. **Root-caused S6.4 (2026-09-24): libdragon, not the engine.** `rdpq_debug` validates each RDP buffer inside the RSP/RDP interrupt handlers with interrupts disabled (`__rdpq_trace_fetch` → `__rdpq_trace_flush`, "FIXME: remove this", unchanged in upstream preview), so on command-heavy frames the vblank interrupt runs up to ~400 half-lines late and the framebuffer flip lands mid-picture: a tear in the lower screen, visible when the picture changes. A Bench = UI boot run with the validator (`make BENCH=1 BENCH_VALIDATOR=1`) reproduced it: 906–952 libdragon `VI WARNING` lines and 0–46 torn flips per 240 frames; no run without the validator has logged a late vblank. S5.1 only hid it (fewer commands per buffer). ares has the late interrupts but draws no tear. Resolution: the validator is a debug tool, so tearing while RDP Check is on is expected and documented (DEBUGGING.md); the engine now counts torn flips (Frame page, `FTP`, `BENCH_PRESENT`). A libdragon report or a local patch (validate outside the interrupt) is optional, the user's call |
| D19 | **Loop pacing jitter.** Under triple buffering the loop is paced by framebuffer availability, not vsync: at a steady 60 FPS iterations alternate ~12.5 / ~21 ms (P1.5). `dt` inherits it (possible micro-stutter) | `src/main.c:189-192, 250` | fixed S6.2: dt from `display_get_delta_time()`; presented frames measured by a vblank handler: 256/256 frames on screen for exactly 1 vblank in the demo while the loop still ranged 12.4–20.9 ms (verified-hw 2026-09-24) |
| D20 | The runtime physics ball never gets a shadow and is never selectable: `selectable_object_count` is snapshotted before billboards and the ball are added, and the shadow loop is bounded by it | `src/scenes/demo_scene.c` (`selectable_object_count` set in `demo_init`; shadow loop in `demo_draw`) | **fixed S7 (verified-hw 2026-09-24):** `SceneObject.flags` (`SCENE_OBJ_CASTS_SHADOW`, `SCENE_OBJ_SELECTABLE`) drive the shadow pass and selection (`scene_find_object()`); the ball casts a shadow and can be selected and held |
...
| D22 | `assets/grass_tex.png` is unused but still packed into the ROM (2.9 KB) | `Makefile` wildcard, `assets/` | fixed S12 (removed; a stale generated `filesystem/grass_tex.sprite` goes with `libdragon make clean`) |
...
| D25 | Render-loop speed depends on code placement. The VR4300 I-cache is 16 KB and direct-mapped: the first S2 triangle loop shared all 46 cache lines of `rdpq_triangle_rsp`, costing ~1,100 cycles per triangle (+38 % CPU in a same-ROM A/B), and unrelated edits moved benchmark CPU by ~6 % between builds. libdragon's `-ftrivial-auto-var-init=pattern` also memset the triangle scratch arrays on every triangle | `Makefile`, `src/engine/hot_text.ld`, `src/render/*.c` | fixed S2 (hot-text block, `tools/hot_text.py` in CI, `ENGINE_NOINIT`); S3: texture uploads run ~7 KB of libdragon code between face groups and cannot be pinned (the mesh phase would need ~17 KB), so `mesh_draw` skips repeat uploads (Textures bench −22 %) and the checker follows callees transitively; S7.1 (D35): the per-object loops around the drawing functions and libdragon's command-buffer switch (every ~20–30 triangles, not a rare path) are pinned and checked too |
| D26 | Data layout moves render timings by up to ~9 %: identical `mesh_draw` code on two copies of the same pillar data differed by 9 % in one ROM (Mesh A/B), most likely 8 KB direct-mapped D-cache aliasing between vertex data and stack scratch | `src/render/mesh.c`, mesh heap allocations | characterised S4 (Bench = Layout): geometry or `Mesh` structs on the render stack's D-cache sets (≈0x1880–0x1CF0) cost 4–8.5 %; S7.1: `mesh_draw` no longer reads the `Mesh` struct per triangle, and with everything else pinned the heap's vertex data is the main layout effect left (~2 % on mesh steps between a plain and a `LAYOUT_PAD=448` build: the pillar's vertices on the pinned camera copy's lines); mitigation → P3.1 (vertex cache: the triangle loop reads one static vertex buffer at a known colour). S9 (2026-09-25): +4 KB of static data moved the heap by 38 KB and the pillar's vertices onto the mesh phase's stack lines and the pinned rspq/rdpq state (colour 0x1BB0–0x21F0): `mesh_tris` +13 %, Bench = All 20 steps +8–11 %, objects 32 60 → 56 FPS → S9.1 places mesh geometry blocks by D-cache colour. **Geometry fixed S9.1 (verified-hw 2026-09-25):** `mesh_finalize()` puts each block at a fixed colour in a window (0x0520–0x0F1F) clear of the render stack and the pinned data, and `hot_data.py` checks the window in CI; mesh steps back to +2.3–3.1 % of S8 (S9: +8–11 %). A `LAYOUT_PAD=448` build still differs by 1–2.5 % on mesh steps and ~6 % on textures: `Mesh` structs (face groups and materials, read per group) and per-object data are placed by their owners → Phase 3 S3 (vertex cache: the triangle loop no longer reads mesh data per corner) and S12 |
| D27 | At boot the scene keeps `lighting_init()`'s ambient (0.15) while the Lighting tab shows Ambient 20 % (0.20): the demo applies menu values only on change and its lighting caches start at the menu defaults, so the menu and the scene disagree until a Lighting item changes | `src/scenes/demo_scene.c` (`last_*` lighting caches, `demo_init`) | **fixed S8 (verified-hw 2026-09-24):** `settings_invalidate()` in `demo_init()` makes the next update apply every option, so the scene matches the menu after boot and after every reset; an atmosphere preset owns sun colour, brightness and ambient (greyed out on the Lighting tab) instead of whichever changed last |
| D28 | Intermittent RSP crash: `rspq_highpri_sync` wait loop times out (status 0x3403) — the RSP sleeps at its idle `break` with HIGHPRI_RUNNING still set. A race between the audio mixer's high-priority work and a low-priority buffer switch in the pinned libdragon; seen twice (P1 validator toggle; S4 after Reset Soak + menu). Fixed upstream in `7c57c409d` (2026-08-23), which needs the rspq rework `cd9d88c64` | `libdragon/src/rspq/rspq.c` (`rspq_next_buffer`) | fixed in S4b.1 (libdragon `39d0d6096` includes `7c57c409d`); verified on the S4b.2 build: no RSP crash in ~40 minutes across several boots (Reset Soak ×4, Menu Sweep, Bench = All, Bench = Audio ×3) |
| D29 | Audio mixing waited behind the frame's RSP work. `snd_update()` ran after `rdpq_detach_show()`, and each mixed buffer is a high-priority RSP job the CPU waits for; there the RSP is still busy with the frame just queued. With music the demo's `audio` slot averaged 2.7 ms (10.2 ms peak) after the libdragon upgrade, which also doubled the buffer rate (25 → 50 per second); Bench = Audio: 2.07 ms per frame, p99 21.5 ms, 1 % low 46 FPS | `src/main.c` frame loop, `src/audio/audio.c` | fixed S4b.2 (mixing right after `display_get()`: 0.48 ms per frame with music, 1 % low 57 FPS; the poll point stays selectable for Bench = Audio) |
| D30 | A sound faster than the mixer's output rate asserted `frequency … exceeds configured limit` when it started. Each channel is limited to the output rate (22,050 Hz) unless raised, and release builds don't check. Opus is always 48 kHz, so the first Opus benchmark stopped at its first Opus step. A 44.1 kHz WAV would have failed the same way, and the old module had the same gap | `src/audio/audio.c` (`snd_init`) | fixed S4b.2 (`snd_init()` reads every sound's rate at boot and raises each channel group's limit to its fastest sound) |
| D31 | libdragon: a channel whose sample ring last served Opus or ULC asserts `samplebuffer too small` when it starts a waveform with another sample unit, such as VADPCM. `mixer_ch_play` resizes the reused ring with the old codec's append margin: `samplebuffer_set_unit_bytes` runs before `samplebuffer_set_waveform` updates `append_units`. Found in ares when the demo's VADPCM track followed the Opus benchmark step on one channel | `libdragon/src/audio/mixer.c` (`mixer_ch_play`) | worked around S4b.2 (a music slot gets a fresh ring whenever a different track starts; effects share one encoding); not yet reported upstream |
| D32 | In a boot-to-benchmark run (`BENCH=1`) on the A3D, step 0 ran ~40 % slower on both CPU and RDP, with the mixer at 4.1 ms instead of 0.09. That step is the first ~5 s after reset; every later step was normal. Cause unknown | boot sequence, `src/main.c` | root-caused and mitigated S6.2 (2026-09-24): measured with constant content (boot-to-benchmark holding the empty scene, `BOOT` rows per second), for ~7.5 s after **every** reset the same work costs ~35 % more CPU, ~47 % more RDP time and ~25 % more audio mixing, then drops to normal within one second. The demo boot shows the same window (s 1–7). Code and content are constant across the step, and ares shows nothing: the A3D or the cart after reset, not the engine. The 4.1 ms mixer spike is gone. Mitigation: boot-to-benchmark runs settle 15 s before step 0 (`BENCH_SETTLE` rows); take no A3D measurement in the first 10 s after reset |
| D33 | S5.1 Bench = All: particles 64–128 +11 to +14 % CPU (`draw`, the particle renderer) and textures 2 +7.8 % (`mesh_light`), in code the stage did not touch. Most likely data layout (static data +11 KB moved every static array; the 128-particle pool spans most of the D-cache); ~0.1 ms is the profiler, on in this run and off in the S4b.2 reference | `src/render/particle_draw.c`, bss layout | S5.2: textures back to −7..−13 % (layout), particles still +11..16 % with the pool clear of the stack sets and the particle update also +10 % (pure CPU): code placement suspected → same-ROM A/B, scheduled in S6 (decided 2026-09-24). S6.1: back to +3–5 % vs S4b.2 with the profiler on (~0.1 ms of it), no particle code changed: layout. A/B in S6.3. **Fixed S6.3 (verified-hw 2026-09-24):** `tools/hot_data.py` showed the particle pool on 47 of the particle loop's stack lines in every build; pinned (`src/engine/hot_data.ld`), particles are 3.5–5.5 % faster than S4b.2 |
| D34 | libdragon's command-queue write pointer (`rspq_cur_pointer` / `rspq_cur_sentinel`, read and written by every rdpq command) lives in static data after all code, so it slides through the 8 KB D-cache when code grows. In one build it sat at set 0x1750, just below the render stack's hot sets (≈0x1880–0x1CF0, D26); 448 bytes of unrelated code (the D32 settle) moved it to 0x1920, onto them: `mesh_tris` +14 %, every mesh-heavy Bench step +7..12 %, RDP unchanged (about 117 extra cycles per triangle) | libdragon `.bss`, `n64.ld` data placement | found S6.2 (2026-09-24, two boot-to-benchmark builds compared with `BENCH_LAYOUT` and `nm`) **fixed S6.3 (verified-hw 2026-09-24):** `src/engine/hot_data.ld` pins the render path's static data (libdragon's rspq/rdpq state, triangle formats, fog, profiler and stats counters, particle pool, shadow scratch, floor grid) to fixed colours away from the stack; `tools/hot_data.py` checks it in CI; with `make LAYOUT_PAD=448` (the D34 shift) Bench = All agrees within ±0.5 % (±1.7 % at the budget edge), where it moved 7–12 % before |
| D35 | The draw path still reads unpinned data whose D-cache colour moves when static data changes size, and runs an unpinned per-object loop. S7 grew `SceneObject` by 16 bytes: `Scene.camera` and `Scene.lighting` (read by `mesh_draw` per object and per face group, through pointers) moved onto the render stack's hot sets, the pillar's `Mesh` (re-read every triangle for the group's index range and the index/vertex pointers) onto the pinned rdpq state, and `bench_draw` onto 36 of the mesh phase's I-cache lines. Bench = All: +21.6 µs per object, +3.7 µs per object per point light, objects 32 60 → 56.8 FPS. `hot_data.py` checks only data the phases name, `hot_text.py` only the phases' callees, and S6.3's `LAYOUT_PAD` check shifted code, never static data | `src/scene/scene.h` (`Scene` layout), `src/render/mesh.c`, `src/scenes/benchmark_scene.c`, `tools/hot_data.py`, `tools/hot_text.py` | found S7 (2026-09-24); **fixed S7.1 (verified-hw 2026-09-24):** the camera and lighting the renderers read are per-frame copies at a pinned colour (`scene_view_camera()` / `scene_view_light()`), `mesh_draw` keeps its per-triangle values in locals, the per-object loops (`ENGINE_HOT_LOOP` / `ENGINE_HOT_HEAD`) and libdragon's command-buffer switch are pinned and checked, `hot_data.py` follows `REACHED` data, and `LAYOUT_PAD` now moves all unpinned code and data. Bench = All: 0 regressions vs S6.3 in both layouts; a `LAYOUT_PAD=448` build agrees within ±1 % on particles and the per-triangle work, ~2 % on mesh steps (heap vertex data, D26) |
| D36 | The presented-frame tracking (S6.2) and the torn-flip check (S6.4) saw each flip one vblank late. The engine's vblank handler read the hardware `VI_ORIGIN`, which libdragon writes only after every vblank handler has run (`vi_show()` updates its register copy). The vblanks-per-frame counts were right (every frame shifted alike), but the torn half-line was the next interrupt's, and S9's lag metric read 3 vblanks where 2 was true, and low-latency pacing ran at 30 FPS, in the first ares runs | `src/engine/engine.c` (`on_vblank`) | found and **fixed S9 (verified-hw 2026-09-25):** the handler reads `vi_read(VI_ORIGIN)`, the framebuffer about to be scanned |
| D37 | Since S9 the particle renderer costs 23.6 µs per particle instead of 20.8–21.5 (S6.3–S8), +13 %; 21.8 µs in a `LAYOUT_PAD=448` build. Every function and variable of the particle phase is pinned, and `hot_text.py` / `hot_data.py` report the same placement as in S8, so the cost follows something the tools don't track. Candidates: interrupt handlers (S9's input core added vblank and SI work), the cold wait path of libdragon's buffer switch (`acct_switch`, `__rsp_check_assert`), and RDRAM placement of the heap (command buffers, framebuffers). Bench = All particles 64–128 +7–9 % vs S8 | `src/render/particle_draw.c`, `mesh_draw`, interrupt handlers, heap | found S9.1 (2026-09-25). S10: libdragon accounts no interrupt time on hardware, but its RSP-wait time (`rsp_wait`) is a constant ~0.3 ms per frame in the particle steps, so the per-particle cost is CPU work. S10 also moved the mesh steps +3–5 % (`mesh_tris` +7 %, ~116 cycles per triangle) while the mesh phase's code shifted uniformly by 160 bytes and its data, geometry and stack colours stayed put. Left: RDRAM placement of the heap (libdragon's command buffers, written uncached for every primitive; framebuffers and Z-buffer) and the interrupt entry code. S12 (Phase 2) logs those addresses in `BENCH_LAYOUT` → Phase 3 S1 (`HEAP_PAD` probe). **S1: not RDRAM placement.** Moving the whole heap 1 MB with identical code left every step within ±0.2 % CPU and every per-primitive slot within ±0.5 % (the noise of the same build booted twice). Against S13, S1 moved `mesh_tris` −6 % with the heap at the same addresses and `mesh_draw` compiled to the same code plus a debug branch. Left: code placement the cache tools don't model, first the interrupt handlers that run while a phase draws → Phase 3 S12 |
| D38 | Near and over the frame budget the audio mix waits for the RSP. At 32 pillars the `audio` slot is 0.1–0.6 ms or 2.2–3.2 ms depending on the build (the step sits at the 60 FPS edge: S7, S9 and S9.1 run at 56–59.7 FPS), 6 ms at 48 pillars and 10 ms at 64 in every build since S6 (25–30 % of the frame), and 2.3–4.2 ms in Bench = Latency's +4 and +8 ms steps. `mixer_poll()` waits for a high-priority RSP job, and the RSP switches to the high-priority queue only between two commands (`RSPQ_Loop`), not while it spins in rdpq's wait for an idle RDP (every frame's `rdpq_clear_z()` runs on the RSP behind an `rdpq_fence()`) or for RDP buffer space | `src/audio/audio.c`, `src/scene/scene.c` (`rdpq_clear_z`), libdragon `rspq` / `rdpq` | found S9.1 (2026-09-25). S10 measured the CPU's RSP waits: 4.0 ms per frame at 32 pillars and 7.2 at 48, of which 3.1 and 6.4 ms in the audio mix and ~0.9 ms at full command buffers (lighter steps: ~60 µs). Next: SP/DP status when the mix starts. Candidates: clear Z with an RDP fill (no fence), mix where the RSP is idle, fewer and larger audio buffers → Phase 3 S2, which also tries libdragon's non-blocking `mixer_try_play()` (the wait tracks the previous frame's RDP time, not only the fence). **Fixed in Phase 3 S2a (2026-09-25).** Cause: after each frame's `SYNC_FULL` libdragon's RSP sends nothing until the RDP has finished that frame (an RDP hardware-bug workaround), and the CPU can run only 2 KB ahead of the RSP, so the first RSP-bound call of the next frame waited for the RDP: the mix, or with an asynchronous mix the triangle submission. Fix: the frame queue (the frame recorded into rspq queues, submitted whole) and the asynchronous mix. `rsp_wait` 0 in every Bench = All step; 48 pillars 40.6 → 60 FPS |
| D39 | A body resting on the ground flipped `grounded` every physics step. Each step's gravity (~16 units/s) came back as a 0.7 × 16 = 11.2 units/s bounce, above the 10 units/s rest threshold. At 60 FPS `physics_body_is_grounded()` was false on every other frame; at 30 FPS (two steps per frame) it was false on every frame the game saw, so a jump gated on it (PHYSICS.md's own example) would miss inputs. Invisible in the demo: the ball stays on the surface, and bounce sounds need 60 units/s | `src/physics/physics.c` (`physics_step`) | found and **fixed S11 (verified-hw 2026-09-25):** an impact slower than two steps of gravity is a resting contact (the normal velocity stops, `grounded` set); host tests check the flag stays set at 60 and 30 FPS and at the NTSC frame times |
| D40 | A menu tab whose items are all disabled would leave the cursor at -1: `find_next_enabled()` returns its start (-1) when no item is enabled, and `menu_change_value()` / `menu_switch_tab()` then read `items[-1]`. No tab reaches it today | `src/ui/menu.c` | found S12 (doc audit) → Phase 3 S12, with a host test |
...
| 3 | Graphics features independent of Tiny3D | **in progress** — §7 (stages S0–S14; S0–S2 2026-09-25) | Vertex cache, Gouraud, sprite animation, CI4/TMEM residency, fonts, VI options, decals, skybox, sorted transparency — each with a BENCHMARKS row |
...
while (1) {
    ...
    PROF_BEGIN(PROF_FRAME);
      ...
      overlay_draw();
      rdpq_detach_show();
      // audio: PROF_BEGIN(PROF_AUDIO); snd_update(dt); PROF_END(PROF_AUDIO); ran here until S4b.2,
      // now right after display_get() (D29, AUDIO.md)
    PROF_END(PROF_FRAME);
...
| P1.7 | verified-hw (revised) | RDP load from hardware counters (93.75 MHz on the A3D); libdragon RSPQ_PROFILE blocked at this commit (IMEM overflow), retried at `39d0d6096` (S4b): the mixer microcode now overflows DMEM by 8 bytes, still blocked |
...
| S4 | P2.4 **Mesh right-sizing** (D8) | ✔ verified-hw 2026-09-23 | `mesh_finalize()` reallocs vertices and indices to their exact size; every builder calls it (cube, mesh_defs, billboard, benchmark boxes). | `mesh.c/h`, `cube.c`, `mesh_defs.c`, `billboard.c`, `benchmark_scene.c` | heap_used after demo init −90 KB |
| S4b | P2.13 **libdragon upgrade + sound module** (pulled forward from P4.0; D28–D31) | ✔ verified-hw 2026-09-23 (S4b.1 libdragon upgrade; S4b.2 sound module, Opus measured, D28 soak passed) | **S4b.1:** move the submodule to current `preview` (≥ `7c57c409d`), `libdragon install`, `libdragon make clean` (asset formats), fix API drift, align `.libdragon/config.json` and CI; retry RSPQ_PROFILE (P1.7: still blocked). **S4b.2:** rewrite the sound module on libdragon's stable mixer API. It adds music crossfades that stop decoding while muted, 8 effect voices with priority stealing, positional effects, and master/music/SFX volume ramps. Mixing moves to right after `display_get()` (D29). Also: Bench = Audio (encoding × poll point), Opus opt-in (`SND_OPUS`), and debug-only ROM data outside `filesystem/`. Tiny3D stays in P4.0. | `libdragon/`, `src/audio/`, `src/main.c`, `Makefile`, `benchmark_scene.c`, AUDIO.md, SETUP.md, HARDWARE.md | CI green; full benchmark: no regression > 5 % unexplained (S4b.1 vs S4, S4b.2 vs S4b.1); A3D sound check; no RSP crash across Reset Soak ×3, Menu Sweep and 30 min of use with the menu |
| S5 | P2.12 **Text & menu performance + UI modularity** (D18 root) | S5.1 ✔ verified-hw 2026-09-24 (UI core, cached menu, styles, scroll bar); S5.2 ✔ verified-hw 2026-09-24 (HUD panels and overlay on cached layers, Classic and Minimal styles, Settings → UI Style); S5.3 ✔ verified-hw 2026-09-24 (dialog system: JSON source compiled by `tools/dialog_build.py` to `.dlg` banks, `DialogRunner` with event/condition/variable hooks, host-tested; `TextBox` with pages, typewriter reveal, pauses, choices, in every UI style; Debug → Dialog; Bench = UI steps 50/51; docs/DIALOG.md) | `text_draw` sets the font style only when the colour changes; the demo HUD becomes one cached paragraph rebuilt at 4 Hz (the overlay's `text_build` pattern); the menu caches its layout and rebuilds only when the cursor, tab or a value changes. | `text.c/h`, `demo_scene.c` (HUD), `menu.c` | `hud` ≤ 0.4 ms (from 1.1); `menu` avg ≤ 1.5 ms, peak ≤ 3 ms with the validator off; menu frames stay under 16.7 ms with the validator on |
| S6 | P2.5 **Engine core + frame pacing** (D9, D18, D19, D33) | S6.1 ✔ verified-hw 2026-09-24 (engine_config.h, engine.c owns init and the loop, main.c = the demo game; ENGINE.md; Bench = All 0 regressions vs S5.2); S6.2 ✔ verified-hw 2026-09-24 (display_set_fps_limit, display_get_delta_time, display_get_zbuf; presented-frame tracking, BENCH_PRESENT and FTP rows, BOOT rows; D19 fixed; 0 regressions vs S6.1; D32 root-caused as the A3D/cart after reset, mitigated with a 15 s settle in boot runs; D34 found); S6.3 ✔ verified-hw 2026-09-24 (hot data pinned: `hot_data.ld`, `engine_ld.awk`, `tools/hot_data.py` in CI, `make LAYOUT_PAD`; D33 and D34 fixed; layout shifts no longer move the benchmark; RAM budget counts pads; PGO added to Phase 3 as experiment P3.14); S6.4 ✔ verified-hw 2026-09-24 (D18 root-caused in libdragon's validator; torn-frame counter; `make BENCH=1 BENCH_VALIDATOR=1`). **S6 complete** | `src/engine/engine_config.h` (screen size, FB count, guard bands, max dt) replaces every hardcoded 320/240 and guard band; `src/engine/engine.c/h` owns init and the frame loop (`main.c` shrinks to setup); `dt` from `display_get_delta_time()`; 30/60 FPS through `display_set_fps_limit()` instead of the busy-wait; Z-buffer from `display_get_zbuf()`. Use OVERLOAD to settle D18 (does a late frame flicker without the validator; does the FPS limit or buffer count change it) and document the answer in HARDWARE.md. | new `src/engine/`, `main.c`, `mesh.c`, `floor.c`, `shadow.c`, `particle.c`, `atmosphere.c`, `camera.c`, `scene.c`, `benchmark_scene.c` | Frame page loop-time histogram single-moded at 60 FPS (D19 gone); 30 FPS mode holds 30 with `limiter` ≈ 0 ms of spinning; D18 root cause written up and fixed or mitigated |
| S7 | P2.6 **Scene owns physics & colliders** (D11, D20) | ✔ verified-hw 2026-09-24 (`Scene.physics`; objects own a collider and a body, synced by `scene_update()`; object flags; the ball casts a shadow and can be selected and held (kinematic bodies); the platform's box collider moves with it; `test_scene.c`; fade works). Bench = All found D35 (layout, not S7's code) → S7.1 | `Scene.physics`; `SceneObject.collider_handle` actually used (`scene_object_set_collider()`, `scene_sync_colliders()`) in place of `obj_colliders[]`; shadow casting and selection driven by per-object flags rather than an index snapshot, so the ball gets a shadow. | `scene.c/h`, `demo_scene.c` | The ball casts a shadow on the A3D; host `test_scene.c` (add/remove/collider sync); Demo↔Benchmark fade still works |
| S7.1 | **Layout-proof per-object draw path** (D35) | ✔ verified-hw 2026-09-24 (as planned, plus libdragon's command-buffer switch pinned: it runs every ~20–30 triangles and had landed on the shadow code in S6.3 and the particle code in S7; no separate `DATA_PAD`: `LAYOUT_PAD` itself now moves all unpinned code and data. Bench = All: 0 regressions vs S6.3 plain and padded; objects 32 at 60 FPS; the padded build agrees within ±1 % on particles and per-triangle work, ~2 % on mesh steps (heap vertex data, D26 → P3.1); Reset Soak 0 B) | The camera and lighting the draw path reads are copied once per frame to a pinned D-cache colour, and every scene draws with the copies; `mesh_draw` keeps the per-triangle values it re-reads (group index range, index and vertex pointers, material type) in locals; the per-object loops around `mesh_draw` and `shadow_draw_*` (benchmark, scene, demo) are pinned in the hot-text block; `hot_data.py` checks the copies against every phase's stack, `hot_text.py` checks the loops against the phases they drive; a static-data layout check (`make DATA_PAD`) joins `LAYOUT_PAD`. | `scene.c/h`, `mesh.c`, `benchmark_scene.c`, `demo_scene.c`, `hot_data.ld`, `hot_text.ld`, `tools/hot_data.py`, `tools/hot_text.py` | Bench = All back within 5 % of S6.3 (objects 32 at 60 FPS); a data-pad build agrees within ±1 % |
| S8 | P2.7 **Settings module** (D10) | ✔ verified-hw 2026-09-24 (`src/ui/settings.c/h`: one table of options, choices and values with static asserts, typed accessors, change tracking; Controls tab generated from the action module; D10 and D27 fixed; an atmosphere preset owns sun colour, brightness and ambient; A3D walk-through, Cancel, Reset Scene and Menu Sweep (25 items, 126 options) clean; Bench = All 0 regressions vs S7.1; CI fails on compiler warnings) | `src/ui/settings.c/h` holds tab/item enums, option strings **and** their value tables side by side; `_Static_assert` on every table size; `menu_add_item` counts come from `ARRAY_LEN`; `demo_scene.c` loses the TAB_/ITEM_ macros and lookup tables (~300 lines). | `settings.c/h`, `main.c`, `demo_scene.c`, `menu.c` | Every menu item still applies and Cancel reverts (A3D walk-through); host test that every value table matches its string table |
| S9 | P2.8 **Input unification** (D12) | ✔ verified-hw 2026-09-25 (extended at the user's request: modular, lowest latency, multiplayer). Input core `src/input/input.c` (joypad, pads, fresh-read wait, vblank sampling of skipped reads, rumble, read timing), pure action layer `action.c` (4 players on any port, priority-ordered context stacks with consume/modal, buttons, chords, stick directions, analog axes with radial deadzones and curves, repeat, held time; `action_chord_pressed()`), `UiInput` for the menu and text box, the engine's UI and debug contexts; the loop polls after `display_get()` and can pace for low latency; input lag measured per presented frame (Frame and new Input overlay pages, `FTP`, `BENCH_PRESENT`, `BENCH_INPUT`, Bench = Latency: 3 / 2 / 1 vblanks for Classic / Low / Lowest); presented frames now detected in the right vblank (D36); `make TOUR=1` screenshot tour; README refreshed. D12 fixed. Bench = All +8–11 % on mesh steps from heap vertex placement (D26) → S9.1 | `ACTION_CTX_MENU` with fixed menu actions; menu navigation, START, the debug shortcuts and benchmark abort read actions instead of the raw joypad; `action_chord_pressed()`; `_Static_assert(MENU_MAX_ITEMS >= ACTION_COUNT)`. | `action.c/h`, `menu.c`, `demo_scene.c`, `debug_menu.c`, `benchmark_scene.c` | Host `test_action.c` covers contexts and chords; menu, D-Up/D-Down and Start behave as before on the A3D |
| S9.1 | **Mesh geometry by colour** (D26) | ✔ verified-hw 2026-09-25, partly met. `mesh_finalize()` places every geometry block in a D-cache window (0x0520–0x0F1F, `engine/hot.h`) clear of the render stack and the pinned data; `hot_data.py` checks the window against the mesh and shadow phases; `scene_init()` restarts the placement, so a scene's meshes take the same colours after every reset. Added: `INPUT_SYNC_AUTO`, the new default, which waits for the vblank's read only while the CPU work is under 70 % of the budget (Bench = Latency: Low keeps up with Classic near the budget, 59.8 vs 59.9 FPS, and keeps 2 vblanks of lag in light scenes). Bench = All vs S9: 0 regressions, mesh steps −6 to −7 %, objects 32 56.1 → 59.7 FPS. vs S8: mesh steps +2.3–3.1 %, but particles +7–9 % (D37) and textures +5–6.5 % (cold upload path, unpinned `Mesh` structs) remain, and a `LAYOUT_PAD=448` build is 2–6 % faster, not within ±1 %. Found D37 and D38 | `mesh_finalize()` places each geometry block (vertices, indices) on D-cache colours clear of the render stack and the pinned render data, so a static-data change no longer moves mesh timings; checked in CI | `mesh_build.c`, `tools/hot_data.py` | Bench = All back to S8 or better, and a `LAYOUT_PAD=448` build within ±1 % |
...
| S4b | `test_audio.c`: voice stealing, positional gains | Bench = All; Bench = Audio; Reset Soak ×3, Menu Sweep, 30-minute soak | music fade-in, menu sounds, bounce panning, Sound tab volumes and mute |
...
| S9.1 | `test_mesh.c` placement (colours in the window, packing, same colours after a reset); `hot_data.py` geometry window | Bench = All plain and `LAYOUT_PAD=448`; Bench = Latency | — |
| S10 | `test_particle.c`: batches by blend mode | validator on; Bench = Particles additive, alpha, mixed | smoke translucency |
| S11 | `test_physics.c`: the demo's ball at 1/60, 1/30 and NTSC frame times (rest time, tunnelling, `grounded` at rest, steps against time) | `BALL` rows (launch to rest) | ball at 30 and 60 FPS |
| S12 | link check; CI | — | — |
...
## 7. Phase 3 — Graphics features independent of Tiny3D — status: in progress (S0–S2 2026-09-25)

Phase 3 starts from what Phase 2 measured. The CPU triangle path, the audio mix's RSP wait (D38) and the layout effects (D26, D37) come first; then a Blender runtime-asset workflow; then the graphics features. Decisions (2026-09-25):
- **Performance first.**
- **Blender content arrives as runtime assets:** JSON exported through the Blender MCP server, built into ROM files the engine loads, previewed in a mesh viewer scene. Level layout export comes later.

Start reference: `docs/benchmarks/2026-09-25-p2-s13-all-bootbench-debug-a3d.csv` (Phase 2 exit).

### 7.1 Candidate list with honest feasibility (written before Phase 2; current status in §7.2)
...
|---|---|---|---|---|
| Post-transform vertex cache (each mesh vertex transformed once, not per triangle corner) | High | No | P3.1 (S3) | vertices are shared only within a face group, and the triangle submission costs more than the transforms: S1 measured submission at 55 % of a pillar's `mesh_tris` and 27 % of a sphere's, so −13 to −21 % is expected on pillars and −16 to −22 % on spheres (the earlier −40–50 % estimate predates the measurement); prerequisite for Gouraud |
| Per-vertex Gouraud lighting (`TRIFMT_ZBUF_SHADE(_TEX)`, `RDPQ_COMBINER_SHADE`/`TEX_SHADE`) | High | No | P3.2 (S5) | `MeshVertex` already stores a normal; +1 normal transform + Blinn-Phong per unique vertex (≈1 ms for 500 verts with 4 lights); smooth normals needed in `mesh_defs` |
| Sprite animation (Feature 8) | High | No | F8 (S7) | sheets: 64×32 RGBA16 = 4 KB fills TMEM → use CI4 (128×32 = 2 KB + 32 B TLUT) or `rdpq_tex_upload_sub` per frame |
| TMEM residency manager + CI4 textures | High | No | P3.3 (S6) | 32×32 CI4 = 512 B + palette: all six cube faces, both billboards and a sprite sheet fit in 4 KB at once → zero per-frame uploads in the demo; `mksprite --format CI4`, `rdpq_tex_multi_begin/end` |
| Texture atlas for billboards/particles | High | No | in P3.3 (S6) | same TMEM math; UV sub-rects |
| TTF fonts via `mkfont` (`rdpq_font_load`) | High | No | P3.6 — **done in Phase 2 S5.1** | `FONT_UI_MONO` / `FONT_UI_VAR` from `assets/fonts/`; still missing: a builtin fallback and a recorded ROM delta |
| VI options: `FILTERS_RESAMPLE_ANTIALIAS(_DEDITHER)`, `GAMMA_CORRECT(_DITHER)`, `rdpq_mode_dithering` | High | No | P3.5 (S11) | menu items + a measured table; AA/dedither cost RDRAM bandwidth, visible in `PROF_WAIT_DISPLAY` / RDP busy |
| Decals & Z modes (`ZMODE_DECAL`, `rdpq_mode_zoverride`) | High | No | P3.4 (S8) | replaces manual Z-bias tricks for shadows; trivial RDP cost |
| Skybox: yaw-scrolled 2-D panorama via `rdpq_tex_blit` | High | No | P3.7 (S9) | replaces 60 fill rectangles; a 256×64 CI4 panorama is 8 KB in ROM; textured 3-D dome is Medium (more tris, fog interaction) |
| Sorted transparency pass (back-to-front translucent objects + alpha particles) | High | No | P3.8 (S8) | insertion sort over ≤ 32 objects per frame; Z-write off for translucents |
| Dynamic / animated point lights + radius culling | High | No | P3.13 (S10) | animation is free; per-vertex lighting makes it look right |
| UV scroll / water | High | No | P3.9 (S11) | per-frame UV offset; a second layer needs a second tile resident (fine with CI4) |
| 2-cycle combiner effects: detail/multi-texture, fake specular map, sphere-map environment (view-space normal → UV on the CPU) | Medium | No | P3.9 (S11, after S5) | two tiles resident; 2-cycle halves fill rate on those triangles — measure with FILLRATE |
| Mipmapping (`mksprite --mipmap`, `rdpq_mode_mipmap`, `tex_mipmaps`) | Medium | No | P3.10 (S11) | 32×32 RGBA16 chain = 2.7 KB; needs 2-cycle and correct LOD (W is already computed); fixes floor shimmer |
| LOD switching (distance-based mesh swap) | High | No | P3.12 (S10) | `MeshLOD { const Mesh *lod[3]; float dist[3]; }` on the object |
| Full-screen fade / colour grade (blended full-screen rectangle) | High | No | P3.11 (S9) | ≈0.5–1 ms RDP; transitions already draw such a rectangle |
| Cheap bloom / motion blur via framebuffer-as-texture (chunked `rdpq_tex_blit`, downsample, additive) | Experimental | No | P3.11 spike (S9) | ≈40 TMEM-sized blits per full-screen pass → 2–4 ms RDP; previous-frame reads need the triple-buffer index; likely 30 FPS only |
| 640×480 interlaced / 512×240; anamorphic widescreen via `vi_set_xscale` | Medium (widescreen: High) | No | option | 640×480×16-bit ×3 = 1.8 MB FB + 614 KB Z → needs the Expansion Pak (the Analogue 3D reports one) and halves the CPU raster budget; widescreen via VI scaling is free |
...
| Profile-guided optimisation (GCC `-fprofile-generate` / `-fprofile-use`) | Experimental | No | P3.14 spike (S13) | the GCC 16 toolchain ships `libgcov` with `__gcov_info_to_gcda` and `-fprofile-info-section`, the embedded path (no file system: counters streamed over ISViewer/USB); profiles can be gathered in ares, since branch counts do not depend on timing. Costs a two-pass build and profiles that go stale with every change; libdragon needs the same treatment for the rdpq path; `n64.ld` does not group `.text.hot` / `.text.unlikely`. Expected gain: a few % in branchy CPU code (update, UI, collision), little in the FP transform |

### 7.2 Where Phase 3 starts (2026-09-25)

What Phase 2's exit run and a code audit showed:
- **The CPU triangle path.** At 16 pillars, `mesh_tris` (transform and submission in `mesh_draw`) is 4.66 of 6.43 ms: about 18 µs, ~1,700 cycles, per drawn triangle. libdragon's `rdpq_triangle_rsp` converts every corner itself and writes ~26 uncached words per triangle (`rdpq_tri.c`), so a vertex cache removes only the engine's transform share.
- **Vertex reuse.** Meshes share vertices only within a face group (flat normals duplicate vertices per face): a pillar's visible groups need ~25 vertices for ~47 corners, a sphere band 14 for 36. That gives the estimate in §7.1.
- **D38.** From 32 pillars up, the audio mix waits 3–10 ms per frame for the RSP. The wait tracks the previous frame's RDP time rather than the Z-clear fence, because the mix is queued before the fence. libdragon's RSP spins on the RDP inside several commands and takes high-priority work only between them. `mixer_try_play()` mixes without waiting while audio is queued, and `AUDIO_BUFFERS 4` is really a latency: 8 buffers, ~160 ms.
- **Layout.** The mesh phase uses 14.5 of the 16 KB I-cache. The shadow scratch's D-cache colours (0x0F20–0x151F) are free while meshes draw. Per-primitive costs still move 5–13 % between builds (D37).
- **Blender MCP** tools load only in Claude Code sessions started after the server is registered. Blender-assisted work runs in a session with Blender open ([BLENDER_MCP.md](BLENDER_MCP.md)).

| Item | Status | Builds on |
|---|---|---|
| P3.1 vertex cache | not started | per-corner transform in `mesh_draw`; the projected shadows' pinned once-per-vertex scratch |
| P3.2 Gouraud | not started | the fog path already uses `TRIFMT_ZBUF_SHADE(_TEX)`, with one colour per triangle; the sphere already has smooth normals |
| P3.3 TMEM + CI4 | partial | repeat uploads skipped within one draw only (Phase 2 S3); RGBA16 only |
| P3.4 decals | not started | shadows use a +0.01 Y offset; the floor a `Z_BIAS` |
| P3.5 VI options | not started | `display_init` in `engine.c`; libdragon's `display_change()` |
| P3.6 TTF fonts | **done (S5.1)** | builtin fallback and ROM delta missing |
| P3.7 skybox | not started | 60 screen-fixed fill strips |
| P3.8 transparency | partial | particles batched alpha → additive (S10); objects not sorted |
| P3.11 post-fx | partial | the scene transition fade |
| P3.9, P3.10, P3.12, P3.13, F8, P3.14 | not started | — |

Bench = All uses 28 of the benchmark's 32 step slots, so Phase 3's measurement kinds run on their own.

### 7.3 Stages

Each stage is one commit (a/b sub-stages allowed) and passes the stage gate of §6.4. Moving baseline: each stage's Bench = All CSV is the next one's comparison point; the exit compares against the start reference.

| Stage | Work | Blender use | Status | Acceptance (A3D unless noted) |
|---|---|---|---|---|
| S0 · Kickoff | Commit the Blender MCP doc and exporter; this rewrite; stale doc numbers | — | ✔ done 2026-09-25 (docs) | link check |
| S1 · Measurement | `BENCH_MESH` kind (pillars and spheres, each also not submitted; mixed shapes with and without fog), `make HEAP_PAD=N` (D37 probe), golden RDP captures (§7.4) | — | ✔ verified-hw 2026-09-25. Submission is 9.1–9.6 µs per triangle: 55 % of a pillar's `mesh_tris`, 27 % of a sphere's. D37 is not RDRAM placement. 18 golden captures cover all four triangle formats. S3's sphere target revised to −15 %. Bench = All vs S13: 0 regressions (BENCHMARKS.md) | transform vs submit split and the D37 verdict recorded; S3's target set |
| S2 · Audio/RSP wait (D38) | split `rsp_wait`, snapshot SP/DP state at the mix; `BENCH_RSP` A/B: Z clear by RDP fill, async mix (`mixer_try_play`), both; `snd_starved` counter. As built: the frame queue (§7.4) | — | S2a ✔ verified-hw 2026-09-25: frame queue + async mix are the defaults, the RDP Z clear was dropped (no gain). Bench = All vs S1: 0 regressions, −11 to −38 % CPU, objects 48 40.6 → 60 FPS, objects 64 30.6 → 40 FPS, `rsp_wait` 0; 0 audio gaps and 0 torn frames in All, Audio, Latency, Overload, UI; soak 0 B. Not met: lag unchanged (up to +1 vblank where the RDP's share is large) → S2b. The exit targets for 32 and 48 pillars are met. S2b ✔ verified-hw 2026-09-25: segmented submission measured and dropped (+3 % CPU, no less lag: the added lag is frames queuing ahead in the triple buffer once the CPU keeps up; Latency = Lowest is unchanged); the queues come from a pool and are released on scene switches (demo heap after the benchmarks 1,172 → 1,026 KB); Bench = All = S2a ±1.7 %; soak 0 B | objects 32/48/64 `audio_us` ≤ 0.3 ms, `rsp_wait` ≤ 1.0 ms; 32 pillars at 60 FPS with CPU ≤ 13.5 ms; no starvation in Audio, Latency, Overload; FILLRATE, lag and torn counts unchanged |
| S3 · Vertex cache (P3.1, D26) | per-group vertex ranges; each visible group's vertices transformed once into a pinned scratch that libdragon's triangle formats read directly (§7.4) | stress meshes once S4 exists | planned | golden RDP captures identical (`rdp_log_to_hex.py --golden`); `mesh_tris` per drawn triangle −12 % on pillars, −15 % on spheres, measured in one ROM against the current path; no step worse than +5 %; `LAYOUT_PAD=448` within ±2 % |
| S4 · Blender runtime assets + viewer | JSON sources → `tools/mesh_build.py` → `rom:/meshes/*.mesh`; `mesh_load()`; exporter JSON mode with smooth normals; mesh viewer scene; `BENCH_ASSETS` (§7.4) | template `.blend`; test meshes authored and exported through MCP | planned | a Blender-made mesh shows in the viewer in ares and on the A3D; loaded pillar within ±2 % of the C pillar in the same ROM; CI builds meshes without Blender |
| S5 · Gouraud (P3.2) | `Material.shading`; per-vertex diffuse into the vertex records, specular per group, point lights culled per object; shaded formats also without fog | smooth test meshes; a Blender render with the same light as a reference | planned | flat path within ±2 % (same ROM); smooth sphere ≤ 1.3× the flat sphere's CPU; demo p99 ≤ 16.8 ms |
| S6 · TMEM + CI4 (P3.3) | per-asset format rule; TLUT in upper TMEM; one TMEM owner across draws; texture-sorted draw list; billboard atlas | palette-limited textures | planned | Textures 4 (CI4) steady at 0 uploads, Textures 8 ≤ 4; demo 0 uploads |
| S7 · Sprite animation (F8) | `anim_sprite.c/h`; frames by sub-surface upload; billboards and particles | rendered sprite sheets | planned | uploads per frame ≤ distinct sheets on screen; host `test_anim.c` |
| S8 · Transparency + decals (P3.8, P3.4) | translucent objects back-to-front with Z-write off, sharing S6's draw list; `ZMODE_DECAL` for shadows | overlapping glass panes | planned | correct from all angles; sort ≤ 0.1 ms at 32 objects; no Z-fighting at grazing angles |
| S9 · Skybox + post-fx (P3.7, P3.11) | yaw-scrolled panorama (gradient kept); colour grade; bloom spike with a go/no-go | rendered panorama strip | planned | sky CPU ≤ 0.3 ms, RDP ≤ 1.5 ms; fade ≤ 1 ms RDP |
| S10 · LOD + animated lights (P3.12, P3.13) | `MeshLOD` distance switching; flickering and moving point lights | LOD chains by Decimate, exported as meshes | planned | Objects 64 at ≥ 45 FPS or drawn triangles −40 %; lighting within ±5 % |
| S11 · Display & material effects (P3.5, P3.9, P3.10) | VI modes through `display_change` (keeps D36's vblank-handler order); UV scroll by tile offset; 2-cycle detail texture; floor mipmaps | water and detail textures | planned | each mode stable for 60 s; cost table; no floor shimmer |
| S12 · Hygiene | the menu's text cache freed while closed and geometry blocks pooled (§6.3 heap items); D40; a Start-menu driver shared by the demo and the viewer; D37 follow-up (the interrupt handlers in `hot_text.py`'s model of each phase); docs audit | — | planned | heap ≤ the start + declared feature costs |
| S13 · PGO spike (P3.14, optional) | as in §7.1 | — | planned | go/no-go recorded |
| S14 · Phase 3 exit | full benchmark and demo measurements against the start reference; stage log; README images including Blender assets | refreshed images | planned | §7.6 |

### 7.4 Stage details: S1–S4

**S1 · Measurement (as built).**
- **`BENCH_MESH`** (not in All): param = variant × 1000 + objects (16, 32, 64), interleaved like Layout, 18 steps.
  - Variants: pillars (0), pillars not submitted (1), spheres (2), spheres not submitted (3), pillars, spheres and textured boxes in turn (4), the same with fog (5).
  - A not-submitted variant runs `mesh_draw()` in full but hands the corners to a `noipa` sink instead of `rdpq_triangle()` (`mesh_debug_set_skip_submit`, debug builds; the flag is read once per draw).
  - Compare `mesh_tris_us` (`bench_compare.py --prof mesh_tris` gives µs per drawn triangle); CPU averages include the D38 wait.
- **`make HEAP_PAD=N`** allocates N bytes first thing in `engine_init()`. N is a variable in `.data`, so builds that differ only in N have identical code and symbols; an empty `asm` keeps the `malloc()`, which GCC deletes when its result is unused.
- **`make BENCH=1 BENCH_KIND=MESH BENCH_RDPLOG=1`** captures one frame of every step in its warm-up (ares). `rdp_log_to_hex.py --tagged` writes the frames and a manifest of triangle counts and a SHA-256 of the triangle commands per step. `--golden` compares a new run with the committed manifest (`docs/benchmarks/2026-09-25-p3-s1-mesh-rdplog-ares.csv`).
- The capture's final `rspq_wait()` timed out in ares, because libdragon prints the log inside its RSP interrupt. `rdp_debug_frame_end()` now waits for the RSP without a time limit first (DEBUGGING.md).
- Results: BENCHMARKS.md, "Phase 3 · S1".

**S2 · Audio/RSP wait.**
- **Diagnose:**
  - `rsp_wait` splits into the wait inside the mix (`audio_wait`) and the rest.
  - Debug builds snapshot the SP and DP status registers, the RDP's current and end pointers, and the RSP program counter when the mix starts.
  - libdragon's RSP profiler is still blocked at this pin (PROFILING.md).
- **`BENCH_RSP`** (not in All) compares, at 32, 48 and 64 pillars:
  - today;
  - the Z-buffer cleared by an RDP fill (colour image set to the Z-buffer, fill mode, one rectangle; no fence);
  - the mix made asynchronous with `mixer_try_play()`;
  - both.
- The winner becomes the default. A `snd_starved` counter (no buffer queued at poll time) guards the audio.
- **As built (S2a).** Round 1 showed the mix was not the cause: made asynchronous, its wait moved into the triangle submission, and the RDP Z clear changed nothing. The RSP was held by libdragon's SYNC_FULL workaround (HARDWARE.md). The fix is the **frame queue**: each frame is recorded into one of two rspq queues and submitted at the end of the frame, so the CPU never writes into libdragon's 2 KB ring while the RSP is held. Drawing code must not wait for the RSP while it records; surfaces are freed with `engine_call_after_rdp()` (ENGINE.md). The asynchronous mix is the default; the RSP probe keeps RSP running / RDP busy / RDP behind (`SP_PC` is not live on the A3D).
- **S2b (as built).** Segmented submission (a pool of queues, each run once; flushes every 8 mesh draws and between the scene's draw phases) was measured on the A3D in one ROM against the single segment: +2.6 to +3.3 % CPU, lag 2.93 → 2.76 vblanks at 48 pillars and unchanged elsewhere, Bench = Latency no better. The RSP cannot forward frame N+1 before the RDP has finished frame N, and the added lag is frames queuing ahead in the triple buffer once the CPU keeps up with the display, which Latency = Lowest already prevents. Segments were dropped; the pool (four queues, lowest finished first) and the release on scene switches stay.

**S3 · Vertex cache.**
- **Group ranges.** `MeshFaceGroup` gains a vertex range. `mesh_finalize()` computes it, keeps each group's vertices contiguous, and splits groups over 32 vertices. That is a no-op for today's builders, and host-tested.
- **Fill loop.** `mesh_draw()` transforms each visible group's vertices once into a pinned scratch, in today's expression order (so results are bit-identical). Records match libdragon's triangle formats, so `rdpq_triangle()` reads them directly:
  - flat: 16 B;
  - textured, or flat with fog: 32 B;
  - textured with fog: 48 B.
- **Rejection.** A per-vertex flag marks the near plane and the guard band. A triangle is rejected when any of its corners is flagged, which is exactly today's rule.
- **Curved groups** keep the winding cull and per-triangle lighting.
- **Scratch placement.** The scratch is 1.5 KB at D-cache colour 0x0F20 (the shadow scratch's, a different phase). The mesh phase stays ≤ 15.3 KB of I-cache so S5 fits.
- **Measurement.** Builds move `mesh_tris` by up to 6 % on their own (S1, D37), so the current per-corner path stays selectable in debug builds, and Bench = Mesh compares both paths in one ROM. The expected gain from S1's split is −13 to −21 % on pillars and −16 to −22 % on spheres.

**S4 · Blender runtime assets and the viewer.**
- **Sources.** `assets/meshes/<name>.json` holds format and version, a source record, `materials` and `groups`:
  - materials: type flat or textured, texture sprite name and size, sRGB base colour, shading flat or smooth, alpha cutout, back-face cull;
  - groups: material, vertices as position, normal and texels, triangles as local indices.
- **Build.** `tools/mesh_build.py` (standard library only, `--selftest`) reads the limits from `mesh.h` and validates finite values, unit normals, degenerate triangles, winding against normals, and texture existence and size. It splits groups to S3's cap and writes `filesystem/meshes/<name>.mesh`.
  - `MSH1` is big-endian: header with CRC32, materials, groups, `MeshVertex` records, 16-bit indices, strings.
  - The Makefile rule follows the dialog one.
- **Load.** `src/render/mesh_load.c` has two entry points:
  - `mesh_parse()` validates, then builds through the builder API. It is compiled into the host tests.
  - `mesh_load()` reads a `rom:/` file.
  - A resolver maps texture names to slots: the scene's texture table, or on-demand sprites in slots 8–15 in the viewer.
- **Exporter.** JSON mode: smooth faces use Blender's corner normals, UVs are scaled by the image's own size, and cutout and culling come from the material. The C output stays.
- **Viewer scene.** Debug → Scene: Viewer lists `rom:/meshes/`.
  - Controls: the stick orbits, C buttons zoom, the D-pad changes the mesh, L/R and Z cycle light and atmosphere presets, and Start returns to the demo.
  - It shows vertices, triangles, planar and curved groups, draw and load time, heap and geometry colour.
- **`BENCH_ASSETS`** measures the C pillar against `pillar.mesh` and the Blender test meshes.
- **CI** runs the dialog and mesh tools' self-tests. `test_mesh_load.c` checks a `pillar.json` round trip bitwise against the C pillar.

### 7.5 Blender integration

- **Blender authors, it never builds.** The committed sources are `assets/meshes/*.json` and PNGs; `.blend` files live in `assets/blender/`. CI and `libdragon make` never need Blender.
- **One rulebook.** `mesh_build.py` enforces the engine's limits and conventions (reading `mesh.h`); the exporter only extracts and pre-checks.
- **The loop** (a Claude Code session with the MCP tools loaded):
  1. Inspect or model in Blender.
  2. Export JSON with `execute_blender_code`.
  3. `libdragon make`.
  4. The viewer in ares (window-only capture), compared with Blender's own render.
  5. The A3D run.
- **Per-stage test content from Blender:** stress meshes (S3), smooth meshes with reference renders (S5), palette-limited textures (S6), sprite sheets (S7), glass panes (S8), a panorama (S9), LOD chains (S10), water and detail textures (S11).
- Level layout export is not in Phase 3; the same asset format can carry it later.

### 7.6 Exit criteria and test plan

**Exit criteria:**
- 32 pillars at 60 FPS with ≥ 4 ms spare; 48 pillars at ≥ 55 FPS.
- Heavy-step audio wait ≤ 0.3 ms.
- The Blender → engine runtime workflow documented and verified on the A3D.
- Every feature stage verified-hw with its metric.
- No benchmark step more than 5 % slower than the start reference (layout checked with `LAYOUT_PAD`).
- Heap within the declared budget; docs updated; CI green.

**Test plan:** the stage gate of §6.4 for every stage. Same-ROM A/B variants decide S1–S3, and S3's triangle commands must match S1's golden RDP captures bit for bit (`rdp_log_to_hex.py --golden`). From S4, the host tests cover the mesh format and loader, and CI runs the tools' self-tests.

| Stage | Automated (host / CI) | Engine tool on the A3D | Visual check |
|---|---|---|---|
| S1 | — | `BENCH_MESH`, `HEAP_PAD` runs, RDP captures | — |
| S2 | — | `BENCH_RSP`; Bench = Audio, Latency, Overload | audio clean at 30 and 60 FPS |
| S3 | `test_mesh.c` group ranges and splits; `hot_data.py` scratch | `BENCH_MESH`, Bench = All, `LAYOUT_PAD=448` | screenshot pair |
| S4 | `test_mesh_load.c`; tool self-tests | `BENCH_ASSETS`; viewer | viewer vs Blender render |
| S5 | `test_lighting.c` | Objects, spheres (same ROM) | smooth shading vs reference |
| S6–S11 | per stage (`test_anim.c`, sort order, LOD selection) | the stage's bench kind | per stage |
| S14 | full CI | Bench = All, UI, Latency; demo measurements | full walk-through |
...
### P4.0 libdragon upgrade + Tiny3D integration (Docker / Windows / CI) — status: planned (the libdragon upgrade itself moved to Phase 2 S4b)
...
| 3 | `BLENDER_MCP.md` (written before S0: the server and the C exporter; the runtime workflow lands in S4) | `RENDERING.md` (vertex cache, Gouraud, transparency, 2-cycle, post-fx spike), `TEXTURES.md` (CI4, residency, mipmaps), `MESH_SYSTEM.md` (group vertex ranges, mesh format and loader, shading flag, LOD), `AUDIO.md` (asynchronous mix), `SCENE_SYSTEM.md` (viewer scene), `EXTENDING.md` (add a Blender mesh), `ARCHITECTURE.md` (skybox), `HARDWARE.md` (VI modes table, D37) |
...
| [Blender](https://www.blender.org/) 5.x + [Blender Lab MCP server](https://www.blender.org/lab/mcp-server/) | authoring meshes and textures, driven from Claude Code ([BLENDER_MCP.md](BLENDER_MCP.md)) | now (Phase 3) |
| [Fast64](https://github.com/Fast-64/fast64) | N64-oriented materials and glTF export from Blender | Phase 4 |
...
Existing: ARCHITECTURE, AUDIO, BENCHMARKS, BILLBOARDS, BLENDER_MCP, CAMERA, COLLISION, DEBUGGING, DIALOG, ENGINE, EXTENDING, HARDWARE, INPUT, MENU_SYSTEM, MESH_SYSTEM, PARTICLES, PHYSICS, PROFILING, RENDERING, ROADMAP (v1), ROADMAP_v2, SCENE_SYSTEM, SETUP, TEXTURES, UI, WORKFLOW. Planned (phase): T3D_INTEGRATION, BLENDER_SETUP, FAST64_MATERIALS, ASSET_PIPELINE, LOD_WORKFLOW (4); ANIMATION, ENTITY_SYSTEM, GAME_FRAMEWORK (5–6).
