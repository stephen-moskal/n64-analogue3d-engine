...
**In progress:** Phase 2, engine hardening (§6): S0–S4b verified on the A3D 2026-09-23 (S4b: libdragon upgrade for the RSP race D28, sound module rework), S5.1 (UI core, cached menu) 2026-09-24; next S5.2 (HUD, overlay, UI styles), then S5.3 (text boxes) and S6.
...
| D33 | S5.1 Bench = All: particles 64–128 +11 to +14 % CPU (`draw`, the particle renderer) and textures 2 +7.8 % (`mesh_light`), in code the stage did not touch. Most likely data layout (static data +11 KB moved every static array; the 128-particle pool spans most of the D-cache); ~0.1 ms is the profiler, on in this run and off in the S4b.2 reference | `src/render/particle_draw.c`, bss layout | open → re-check with S5.2; same-ROM A/B if it persists |
