...
The frame's drawing is recorded into an rspq queue (libdragon's `rspq_queue_*`, preview API) and handed to the RSP in one command at the end of the frame, so the CPU records frame N+1 while the RSP and RDP finish frame N. The queues come from a pool of four: each frame takes the lowest-numbered queue the RSP has finished (a syncpoint follows every run), so only as many queues grow as there are frames in flight, usually two.
...
- **Memory:** each queue keeps the size of the largest frame it has recorded (about 100 bytes per triangle: ~128 KB at 1,000 triangles). `engine_frame_queue_release()` (`frame_queue.h`) destroys them; the scene manager calls it when it switches to another scene (not on a soft reset), so a heavy scene's queues do not stay allocated. The demo carries ~61 KB of queues (S2b).
- **Latency:** where the CPU now keeps up with the display, frames queue ahead in the triple buffer and each adds a vblank of input lag (Bench = Latency: Classic and Low with a 4–8 ms burn 2.2–2.4 → 2.9–3.0 vblanks; objects 48 2.6 → 2.8–2.9 at 60 instead of 41 FPS), while other steps gain (Bench = UI 2.8–2.9 → 2.0). Latency = Lowest (low-latency pacing: no frame ahead) is unchanged at 1.0 / 2.0 / 2.0. Submitting the frame in segments (every 8 mesh draws, S2b) did not shorten it and cost 3 % CPU, because the RSP cannot forward frame N+1 before the RDP finishes frame N anyway; it was dropped.
